...
static int detect_fasterrcnn(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    tmtool::Mat in = tmtool::Mat::from_pixels_resize(bgr.data, tmtool::Mat::PIXEL_BGR, bgr.cols, bgr.rows, w, h);

    const float mean_vals[3] = { 102.9801f, 115.9465f, 122.7717f };
    in.substract_mean_normalize(mean_vals, 0);

    tmtool::Mat im_info(3);
    im_info[0] = h;
    im_info[1] = w;
    im_info[2] = scale;

    // step1, extract feature and all rois
    tmtool::Extractor ex1 = fasterrcnn.create_extractor();

    ex1.input("data", in);
    ex1.input("im_info", im_info);

    tmtool::Mat conv5;// feature
    tmtool::Mat rois;// all rois
    ...
    {
        tmtool::Extractor ex2 = fasterrcnn.create_extractor();

        tmtool::Mat roi = rois.channel(i);// get single roi
        ex2.input("conv5", conv5);
        ex2.input("rois", roi);

        tmtool::Mat bbox_pred;
        tmtool::Mat cls_prob;
...
int fasterrcnn(int argc, char** argv)
{
    if (argc != 2)
    {
