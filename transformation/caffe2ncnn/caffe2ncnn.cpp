...
int caffe2ncnn(const char* prototxt, const char* model, const char* param, const char* bin, const char* table, int quantizelevel)
{
    const char* caffeproto = prototxt;
    const char* caffemodel = model;
    const char* ncnn_prototxt = param;
    const char* ncnn_modelbin = bin;
    const char* int8scale_table_path = table;
    int quantize_level = quantizelevel;

    if (quantize_level != 0 && quantize_level != 256 && quantize_level != 65536) {
        fprintf(stderr, "only support quantize level = 0, 256, or 65536");
        return -1;
    }
    ...
    for (int i=0; i<layer_count; i++)
    {
        ...
        else if (layer.type() == "InnerProduct")
        {
            ...
            for (int j=0; j<binlayer.blobs_size(); j++)
            {
                ...
                if (j == 0)
                {
                    //modified by lyz 20190518 weights of FC layer do not change to int8 
                    if (int8_scale_term)
                    {
                        if (quantize_level == 0)
                        {
                        ...
                        {
                            printf("quantize_level == 256 yes int8_scale_term\n");
                            quantize_tag = quantize_weight((float *)blob.data().data(), blob.data_size(), weight_int8scale, int8_weights);
                        }
                    }
                    else if (quantize_level == 256)
                    {
                        printf("quantize_level == 256 no int8_scale_term\n");
                        printf("data_size == %d\n",blob.data_size());
                        quantize_tag = quantize_weight((float *)blob.data().data(), blob.data_size(), quantize_level, quantize_table, quantize_index);
                    }
                    else if (quantize_level == 65536)
                    ...
                    if (quantize_tag)
                    {
                        ...
                        if (int8_scale_term)
                        {
                            ...
                            {
                                printf("quantize_level == 256 yes int8_scale_term write\n");
                                fwrite(int8_weights.data(), sizeof(signed char), int8_weights.size(), bp);
                            }
                        }
                        else if (quantize_level == 256)
                        {
                            // write quantize table and index
                            printf("quantize_level == 256 no int8_scale_term write\n");
                            printf("quantize_table.size == %lud\n",quantize_table.size());
                            printf("quantize_index.size == %lud\n",quantize_index.size());
                            fwrite(quantize_table.data(), sizeof(float), quantize_table.size(), bp);
                            fwrite(quantize_index.data(), sizeof(unsigned char), quantize_index.size(), bp);
                        }
                        else if (quantize_level == 65536)
                ...
                {
                    // write original data                    
                    int writeLen = fwrite(blob.data().data(), sizeof(float), blob.data_size(), bp);
                    printf("write original data len %d\n",writeLen);
                }
            }
...
//                 float scale;
        ...
        else if (layer.type() == "Upsample")
        {
            ...
            fprintf(pp, " 0=%d", upsample_param.stride());
            fprintf(pp, " 1=%f", upsample_param.scale());
        }
