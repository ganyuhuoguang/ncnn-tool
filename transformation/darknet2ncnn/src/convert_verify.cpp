...
float get_layer_output_diff(tmtool::Mat &darknet_out, tmtool::Mat &ncnn_out)
{
...
void print_layer_output_diff(tmtool::Mat &darknet_out, tmtool::Mat &ncnn_out)
{
  int feature_size = ncnn_out.h * ncnn_out.w;
  float *darknet_ptr = darknet_out;
  for (int c = 0; c < ncnn_out.c; c++)
...
void print_conv_weights(network *net, int i, CustomizedNet &classifier)
{
  ...
  tmtool::Mat darknet_weights(conv.nweights, conv.weights);
  tmtool::Convolution *ncnn_conv =
      (tmtool::Convolution *)classifier.get_layer_from_name(name);
  ...
  if (conv.batch_normalize)
  {
    ...
    tmtool::BatchNorm *ncnn_batch =
        (tmtool::BatchNorm *)classifier.get_layer_from_name(name);

    tmtool::Mat darknet_scales(conv.n, conv.scales);
    diff = get_layer_output_diff(darknet_scales, ncnn_batch->slope_data);
    printf("%s : slope diff : %f\n", name, diff);

    tmtool::Mat darknet_means(conv.n, conv.rolling_mean);
    diff = get_layer_output_diff(darknet_means, ncnn_batch->mean_data);
    printf("%s : mean diff : %f\n", name, diff);

    tmtool::Mat darknet_variance(conv.n, conv.rolling_variance);
    diff = get_layer_output_diff(darknet_variance, ncnn_batch->var_data);
    printf("%s : variance diff : %f\n", name, diff);

    tmtool::Mat darknet_biases(conv.n, conv.biases);
                    ...
                    char *ncnn_modelbin, char *imagepath)
{
  ...
  if (NULL == imagepath)
  {
    in = tmtool::Mat(net_p->w, net_p->h, net_p->c);
    in.fill(0.5f);
  }
  else
  {
  ...
  tmtool::Extractor ex = classifier.create_extractor();
  ...
  for (int i = 0; i < net_p->n; ++i)
  {
    ...
    tmtool::Mat darknet_out(l.outputs, l.output);
...
int convert_verify(int argc, char *argv[])
{
  if (argc < 5)
  {
    printf(
