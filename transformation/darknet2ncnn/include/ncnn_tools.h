...
#ifndef _TMTOOL_TOOLS_H
#define _TMTOOL_TOOLS_H
...
static void print_mat(tmtool::Mat &mat)
{
...
static tmtool::Mat get_input_mat_from_image(const char *filename, int net_h,
                                          int net_w)
{
  cv::Mat letter = get_input_opencv_mat_from_image(filename, net_h, net_w);

  tmtool::Mat darknet_input = tmtool::Mat::from_pixels(
      letter.data, tmtool::Mat::PIXEL_RGB2BGR, net_w, net_h);
...
class CustomizedNet : public tmtool::Net
{
public:
  CustomizedNet() : tmtool::Net(){};

  tmtool::Layer *get_layer_from_index(int index)
  {
    if (index < 0)
    {
      return NULL;
  ...
  tmtool::Layer *get_layer_from_name(const char *layer_name)
  {
  ...
  tmtool::Blob *get_blob_from_name(const char *blob_name)
  {
  ...
  tmtool::Blob *get_last_layer_output_blob()
  {
    int layer_index = layers.size() - 1;
    if (layer_index < 0)
    {
