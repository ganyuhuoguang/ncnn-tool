...
#ifndef __MTCNN_TMTOOL_H__
#define __MTCNN_TMTOOL_H__
#include "net.h"
...
public:
	...
	void detect(tmtool::Mat& img_, std::vector<Bbox>& finalBbox);
	void detectMaxFace(tmtool::Mat& img_, std::vector<Bbox>& finalBbox);
	//  void detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles);
private:
	void generateBbox(tmtool::Mat score, tmtool::Mat location, vector<Bbox>& boundingBox_, float scale);
