...
void MTCNN::generateBbox(tmtool::Mat score, tmtool::Mat location, std::vector<Bbox>& boundingBox_, float scale) {
	const int stride = 2;
...
void MTCNN::PNet(float scale)
{
	...
	tmtool::Extractor ex = Pnet.create_extractor();
	ex.set_light_mode(true);
	//sex.set_num_threads(4);
	ex.input("data", in);
	tmtool::Mat score_, location_;
...
void MTCNN::RNet() {
	...
	for (vector<Bbox>::iterator it = firstBbox_.begin(); it != firstBbox_.end(); it++) {
		...
		tmtool::Extractor ex = Rnet.create_extractor();
		//ex.set_num_threads(2);
		ex.set_light_mode(true);
		ex.input("data", in);
		tmtool::Mat score, bbox;
...
void MTCNN::ONet() {
	...
	for (vector<Bbox>::iterator it = secondBbox_.begin(); it != secondBbox_.end(); it++) {
		...
		tmtool::Extractor ex = Onet.create_extractor();
		//ex.set_num_threads(2);
		ex.set_light_mode(true);
		ex.input("data", in);
		tmtool::Mat score, bbox, keyPoint;
		ex.extract("prob1", score);
...
void MTCNN::detect(tmtool::Mat& img_, std::vector<Bbox>& finalBbox_) {
	img = img_;
	img_w = img.w;
	img_h = img.h;
...
void MTCNN::detectMaxFace(tmtool::Mat& img_, std::vector<Bbox>& finalBbox) {
...
//    tmtool::Mat ncnn_img = tmtool::Mat::from_pixels(img.data, tmtool::Mat::PIXEL_BGR2RGB, img.cols, img.rows);
